	...
	while True:
		...
		if msg == "/viewRequests":
			...
			if response == "/sendingData":
				...
				if data == set():
					print("0 Active requests to join")
				else:
					print("Requests left to respond:")
					for element in data:
						print(element)
			else:
				print(response)
			...
			serverSocket.send(bytes(".","utf-8"))
			data = pickle.loads(serverSocket.recv(1024))
			print("Group members who are online:")
			for element in data:
				print(element)
			...
			if response == "/proceed":
				state["inputMessage"] = False
				print("Provide the username of the new admin: ")
				with state["inputCondition"]:
					state["inputCondition"].wait()
				state["inputMessage"] = True
				serverSocket.send(bytes(state["userInput"],"utf-8"))
			...
			state["alive"] = False
			state["inputMessage"] = False
			print("You have been removed due to your wrong behaviour. Press any key to quit the screen")
			break
		else:
			print(msg)
...
def main():
	if len(sys.argv) < 3:
		print("How to use? jsut type the following command in terminald  : python client.py <IP> <Port>")
		print("Demo: python client.py localhost 5000")
		return
	...
	state["username"] = input("Welcome to Chat Room! Please enter your username: ")
	...
	if response == "/adminReady":
		print("Hola! new group created ",state["groupname"],"You are the admin. Use power wisely!!")
		state["alive"] = True
	elif response == "/ready":
		print("Welcome to ",state["groupname"])
		state["alive"] = True
	elif response == "/wait":
		print("Your request has been sent to the admin to approve")
	...
	while True:
		if state["alive"] or state["joinDisconnect"]:
			break
	if state["alive"]:
		print("Available Commands:\n/1 -> View Join Requests (Only for Admins)\n/2 -> Approve Join Requests (Only for Admin)\n/3 -> Disconnect\n/4 -> View All Members\n/5 -> View Online Group Members\n/6 -> Transfer Adminship\n/7 -> Check Group Admin\n/8 -> Kick Member(Only for admins)\nType anything else to send a message")
