...
def chatRoom(client, username, groupname):
	while True:
		...
		if msg == "/viewRequests":
			...
			if username == groups[groupname].admin:
				...
				if usernameToApprove in groups[groupname].joinRequests:
					...
					print("Member request has been approved with follwoing identity:",usernameToApprove,"| Group:",groupname)
			...
			print("User Left:",username,"| Group:",groupname)
			break
...
def handshake(client):
	...
	if groupname in groups:
		...
		threading.Thread(target=chatRoom, args=(client, username, groupname,)).start()
	else:
